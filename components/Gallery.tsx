...
export default function Gallery() {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      setTrail(prev => {
        const next = [...prev, { id, x, y, rotation, photo }];
  ...
  return (
    <section
      ...
      style={{
        ...
        overflow: "hidden", background: "var(--creme2)",
        cursor: "none",
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 10, padding: "3.5rem 5rem 0", display: "flex", alignItems: "flex-start", justifyContent: "space-between", pointerEvents: "none" }}>
        <div>
          <h2 style={{
            ...
            color: "var(--texte)", lineHeight: .9, letterSpacing: "-.03em",
          }}>
            PHOTOS /<br />SOUVENIRS
          </h2>
      ...
      {/* Polaroïds */}
      {trail.map((p, i) => {
        const age     = trail.length - 1 - i;          // 0 = le plus récent
        const opacity = 1 - (age / MAX_TRAIL) * 0.75;  // 1 → 0.25
        const scale   = 1 - (age / MAX_TRAIL) * 0.20;  // 1 → 0.80
        ...
        return (
          <div
            ...
            style={{
              ...
              width: "220px",
              background: "#fff",
              boxShadow: "0 20px 60px rgba(0,0,0,.22), 0 4px 12px rgba(0,0,0,.12)",
          ...
          >
            ...
            <p style={{
              ...
              color: "rgba(26,26,26,.6)", marginTop: "10px",
            }}>
              {p.photo.caption}
            </p>
          </div>
