  ...
  return (
    <>
      ...
      >
        ...
        {/* Centre — nav pills (desktop uniquement) */}
        <nav className="nav-desktop" style={{ display: "flex", gap: ".3rem", background: "rgba(255,255,255,.08)", border: "1px solid rgba(255,255,255,.15)", backdropFilter: "blur(12px)", borderRadius: "100px", padding: ".28rem .35rem" }}>
          {links.map(l => (
            <a key={l.label} href={l.href}
              style={{ fontFamily: "var(--f-sans)", fontSize: ".82rem", color: "rgba(255,255,255,.8)", textDecoration: "none", padding: ".6rem 1.4rem", borderRadius: "100px", transition: "background .18s, color .18s" }}
        ...
        {/* Droite — bouton Réserver (desktop) + burger (mobile) */}
        <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center" }}>
          {/* Réserver — desktop uniquement */}
          ...
          <button onClick={() => setOpen(o => !o)} className="nav-burger"
            style={{ background: "none", border: "1px solid rgba(255,255,255,.25)", padding: ".55rem .75rem", cursor: "pointer", display: "flex", flexDirection: "column", gap: "5px", borderRadius: "4px" }}
          >
          ...
          >
            {links.map(l => (
              <a key={l.label} href={l.href} onClick={() => setOpen(false)}
                style={{ fontFamily: "var(--f-sans)", fontWeight: 500, fontSize: "clamp(1.6rem, 7vw, 2.4rem)", color: "rgba(255,255,255,.85)", textDecoration: "none", letterSpacing: "-.02em" }}
              >{l.label}</a>
            ))}
            <a href="#reservation" onClick={() => setOpen(false)}
              style={{ marginTop: "1rem", fontFamily: "var(--f-sans)", fontWeight: 500, fontSize: ".88rem", background: "#fff", color: "#0a0a0a", textDecoration: "none", padding: ".75rem 2.5rem", borderRadius: "100px" }}
            >Réserver</a>
          </motion.div>
        )}
      </AnimatePresence>
